...
func _validate_level_grid_renderer_defaults() -> void:
	...
	_assert(ball_renderer.source_rect_for_ball_type(GameSessionScript.BALL_TYPE_FIREBALL, GameSessionScript.BALL_SIZE, 0) == Rect2(Vector2(1, 153), Vector2(18, 18)), "fireballs draw from the original second Balls atlas row")
	_assert(ball_renderer.source_rect_for_ball_type(GameSessionScript.BALL_TYPE_FIREBALL, 26.0, 2) == Rect2(Vector2(57, 173), Vector2(26, 26)), "fireball animation frames stay in the Balls fireball row")
	_assert(ball_renderer.source_rect_for_ball_type(GameSessionScript.BALL_TYPE_NON_STRICKED, GameSessionScript.BALL_SIZE, 0) == Rect2(Vector2(1, 293), Vector2(18, 18)), "non-stricked balls draw from the original third Balls atlas row")
	...
	_assert(BallRendererScript.modulate_for_ball_type(GameSessionScript.BALL_TYPE_FIREBALL) == Color.WHITE, "fireballs preserve the original Balls atlas colors")
	_assert(BallRendererScript.modulate_for_ball_type(GameSessionScript.BALL_TYPE_NON_STRICKED) == Color.WHITE, "non-stricked balls preserve the original Balls atlas colors")
	_assert(BallRendererScript.source_size_for_ball_type(GameSessionScript.BALL_TYPE_FIREBALL, GameSessionScript.BALL_SIZE) == GameSessionScript.BALL_SIZE, "fireball source size follows the gameplay ball size")
	_assert(BallRendererScript.visual_rect_for_ball_type(GameSessionScript.BALL_TYPE_FIREBALL, Rect2(Vector2(10, 20), Vector2(18, 18))) == Rect2(Vector2(10, 20), Vector2(18, 18)), "fireballs draw at the gameplay ball footprint")
	_assert(BallRendererScript.FIREBALL_EFFECT_MODULATE == Color.WHITE, "ball tracks preserve the extracted Fb sheet colors and alpha")
